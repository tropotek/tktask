...
class Register
{
    ...
    public function doDefault(Request $request)
    {
        ...
        $this->form->appendField(new Field\Password('password'))
            ->setAttr('autocomplete', 'off')->setRequired();
        $this->form->appendField(new Field\Password('confPassword'))->setLabel('Password Confirm')
            ->setAttr('autocomplete', 'off')->setRequired();
        ...
        $this->getForm()->appendField(new Action\Submit('register', [$this, 'onSubmit']))->addCss('w-100');
    ...
    public function onSubmit(Form $form, Action\ActionInterface $action)
    {
        ...
        if (($token + 60*2) < time()) { // submit before form token times out
            $form->addFieldError('confPassword', 'Invalid form submission, please try again.');
            return;
        }
        ...
        $this->getFactory()->getSession()->getFlashBag()->add('success', 'Please check your email for instructions to activate your account.');
