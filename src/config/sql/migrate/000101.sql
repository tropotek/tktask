...
(
  ...
  modified TIMESTAMP ON UPDATE CURRENT_TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  created TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_domain__company_id FOREIGN KEY (company_id) REFERENCES company (company_id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE IF NOT EXISTS domain_ping
(
  domain_ping_id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  domain_id INT UNSIGNED NOT NULL DEFAULT 0,
  status BOOL NOT NULL DEFAULT FALSE,
  site_name VARCHAR(256) NULL,
  site_time TIMESTAMP NULL,
